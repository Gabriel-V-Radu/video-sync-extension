  ...
  if (player) {
    ...
    const checkPlayer = setInterval(() => {
      ...
      if (player) {
        ...
        console.error('YouTube player not found after 30 seconds');
        clearInterval(checkPlayer);
      }
    }, 1000);
  }
...
function handleEnableSync(message, sendResponse) {
  ...
  console.log('YouTube player: Waiting for player detection...');
  ...
  const checkInterval = setInterval(() => {
    if (player || Date.now() - waitStart >= maxWait) {
      ...
      if (player) {
        console.log('YouTube player: Player detected during wait');
        enableSyncWithPlayer(message);
        sendResponse({ success: true, playerReady: true });
      } else {
        console.log('YouTube player: Player not found after waiting (page may not have a video player)');
...
function enableSyncWithPlayer(message) {
  ...
  outboundActionSeq = 0;
  lastAppliedSyncSeq = 0;
  console.log(`YouTube player enabled as ${isPrimaryPlayer ? 'PRIMARY' : 'SECONDARY'}`);
